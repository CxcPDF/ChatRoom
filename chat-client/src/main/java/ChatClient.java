    ...
    private class ReceiverHandler implements Runnable {
        ...
        public void run() {
            try {
                while (connected) {
                    int size = 0;
                    selector.select();
                    for (Iterator<SelectionKey> it = (Iterator<SelectionKey>) selector.selectedKeys(); it.hasNext(); ) {
                        ...
                        if (selectionKey.isReadable()) {
                            ...
                            while ((size = clientChannel.read(buf)) > 0) {
                                buf.flip();
                                outputStream.write(buf.array(), 0, size);
                                buf.clear();
                            }
        ...
        private void handleResponse(Response response) {
            ...
            switch (header.getType()) {
                ...
                case FILE:
                    //TODO 文件下载有时间再做
                    System.out.println("文件下载功能暂时不可用，请稍候重试！");
                    break;
            }
        }
